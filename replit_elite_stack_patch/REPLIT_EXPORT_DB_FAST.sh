...
MAIN_BYTES=$(stat -c%s "$MAIN_DB" 2>/dev/null || stat -f%z "$MAIN_DB")
if [ "$MAIN_BYTES" -lt 500000000 ]; then
  log "WARN: main db only $((MAIN_BYTES/1024/1024))MB — expected ~2000-3200MB for full history!"
  log "WARN: workspace may have been truncated. Run REPLIT_FIND_LOST_DB.sh before uploading."
fi
...
if [ "${INCLUDE_LEGACY:-0}" = "1" ] && [ ! -f "$LEGACY" ]; then
  log "INCLUDE_LEGACY=1 but MISSING $LEGACY (folder deleted or never restored)"
elif [ -f "$LEGACY" ] && [ "${INCLUDE_LEGACY:-0}" = "1" ]; then
  LEGZIP="bacbo_legacy_db_${STAMP}.zip"
  log "INCLUDE_LEGACY=1 zipping $LEGACY ($(ls -lah "$LEGACY" | awk '{print $5}')) ..."
  zip -0 -q "$LEGZIP" "$LEGACY"
  ln -sf "$LEGZIP" bacbo_legacy_db.zip
  log "done $LEGZIP $(ls -lah "$LEGZIP" | awk '{print $5}')"
elif [ -f "$LEGACY" ]; then
  log "legacy exists $(ls -lah "$LEGACY" | awk '{print $5}') — set INCLUDE_LEGACY=1 to zip it"
else
  log "no legacy db at $LEGACY"
fi
