...
mkdir -p bot
echo "-- download fix_bacbo_syntax.py --"
curl -fsSL --connect-timeout 20 --max-time 60 -o bot/fix_bacbo_syntax.py \
  "${RAW}/replit_elite_stack_patch/bot/fix_bacbo_syntax.py?v=${V}"
echo "-- download lux_re_harden.py --"
curl -fsSL --connect-timeout 20 --max-time 60 -o bot/lux_re_harden.py \
  "${RAW}/replit_elite_stack_patch/bot/lux_re_harden.py?v=${V}" || true

echo "-- run fix_bacbo_syntax.py --"
